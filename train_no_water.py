...
label_name = dict(zip(labels,['Cultivated land','Forest and grassland', 'built up land']))
...
def reclass(x):
    if x == 10:
        ...
        return 0 ## mask value
def sample_data(im_array,lc_array,batch_array,batch_num = 1,n=9):
    global labels
    batch_size = np.sum(batch_array == batch_num)
    X = np.zeros((batch_size,n,n,im_array.shape[0]))
    ...
    for i in range(indexes.shape[0]):
        if indexes[i].min()>50 and indexes[i,0]<im_array.shape[1]-50 and indexes[i,1]<im_array.shape[2]-50:
            for j in range(X.shape[-1]):
                if n%2 == 0:
                    X[i,:,:,j] = im_array[j,indexes[i,0]-n//2:indexes[i,0]+n//2,indexes[i,1]-n//2:indexes[i,1]+n//2]
                else:
                    X[i,:,:,j] = im_array[j,indexes[i,0]-n//2:indexes[i,0]+n//2+1,indexes[i,1]-n//2:indexes[i,1]+n//2+1]
...
if __name__ == '__main__':
    ...
    batch_array.tofile('batch_array_no_water_9.txt',sep=',')
...
#    plt.show()
    ...
    im_shape = (9,9)
    npp_class = 3
    X,y = sample_data(im_array,lc_array,batch_array,1,int(im_shape[0]))
...
#    plt.imshow(batch_array)
    ...
    ## conv1 layer
    W_conv1 = tf.Variable(tf.truncated_normal([3,3,3,1,128],stddev=0.1))
    ...
    ## conv2 
    W_conv2 = tf.Variable(tf.truncated_normal([3,3,2,128,256],stddev = 0.1))
    ...
    h_pool2 = tf.nn.max_pool3d(h_relu2,[1,3,3,3,1],[1,3,3,3,1],'SAME', name = 'h_pool2')   ## output size -1, 2*12*12,128
    ## conv3 
    ...
    #
    ## fc1
    W_fc1 = tf.Variable(tf.truncated_normal([256,256],stddev=0.1))
    B_fc1 = tf.Variable(tf.constant(0.1,shape=[256]))
    h_pool_flat = tf.reshape(h_pool2,[-1,256])
    ...
    ## last layer
    W_fc = tf.Variable(tf.truncated_normal([256,3],stddev=0.1))
    B_fc = tf.Variable(tf.constant(0.1,shape=[3]))
...
##    plt.ion()
    for e in range(100):
        for b in range(1,batch_num+1):
            i = e * batch_num + b    
            batch_x, batch_y = sample_data(im_array,lc_array,batch_array,b,int(im_shape[0]))
